...
def is_class_or_protein_family(entity):
    return is_class(entity) or is_protein_family(entity)
...
def find_focus_constraint(entity_matches, question, verbose = False):
    ...
    probable_class_entities = [x[0] for x in final_entities if is_class_or_protein_family(x[0])]
    probable_non_class_entities = [x[0] for x in final_entities if not is_class_or_protein_family(x[0])]
